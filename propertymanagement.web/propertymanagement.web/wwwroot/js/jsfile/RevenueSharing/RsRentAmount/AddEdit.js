...
$(document).ready(function () {
    ...
    setDate()
    getAjaxDataChangeList("/Marketing/RevenueSharing/RsRentAmount/GetDataChangeTypeAll", '');
    ...
    function editRowDp(rowElement, downPaymentId) {
        ...
        //document.getElementById('vatModal').value = rowData[2];
        document.getElementById('amountInclModal').value = rowData[3];
        ...
        $('#save_item_dp').off('click').on('click', function () {
            row.data([
                ...
                document.getElementById('amountExclModal').value * 0.11,
...
function openPSM() {
    ...
    getAjaxDataPopupPsm("", "psm_table_header", "/Marketing/RevenueSharing/RsRentAmount/GetDataPSMAll", AddNew);
}
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            response.response.data.forEach(function (item) {
                ...
                if (selectedId != '' && item.paramId == selectedId) {
...
function AddNew() {
    window.location.href = '/Marketing/RevenueSharing/RsRentAmount/Create';
}
...
function setValue(row) {
        ...
        if (row.hasOwnProperty(key)) {
            ...
            if (element) {
                ...
                if (['chargeDateFrom', 'chargeDateTo', 'handOverDate', 'endDateKSM'].includes(key)) {
                    const formattedDate = formatDate(row[key]);
    ...
    getAjaxDataChangeList("/Marketing/RevenueSharing/RsRentAmount/GetDataChangeTypeAll", row.chargeTypeId);
}
...
function countOutstanding() {
    ...
    let compansasiVat = compansasiAmountExcl * 0.11;
    let compansasiAmountIncl = compansasiAmountExcl + compansasiVat;

    document.getElementById('compansasiVat').value = compansasiVat;
    document.getElementById('compansasiAmountIncl').value = compansasiAmountIncl;

    document.getElementById('outstandingAmountExcl').value = rentAmountExcl - dpAmountExcl - compansasiAmountExcl;
    document.getElementById('outstandingVat').value = rentVat - dpVat - compansasiVat;
    document.getElementById('outstandingAmountIncl').value = rentAmountIncl - dpAmountIncl - compansasiAmountIncl;
}

function backToList() {
    window.location.href = '/Marketing/RevenueSharing/RsRentAmount';
}