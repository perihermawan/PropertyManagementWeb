$(document).ready(function () {
    getAjaxDataRentAmount("", "rs_rent_amount_table_header", "RsRentAmount/GetDataAll", AddNew);
    ...
    function applyFilter(e) {
        ...
        if ($.fn.DataTable.isDataTable($('#rs_rent_amount_table_header'))) {
            $('#rs_rent_amount_table_header').DataTable().destroy();
        }

        getAjaxDataRentAmount(data, "rs_rent_amount_table_header", "RsRentAmount/GetDataAll", AddNew);
    }
...
var table = document.getElementById("rs_rent_amount_table_header");

function rs_rent_amount_table_header_editor_view(e) {
    var rowId = table.rows[e.parentNode.parentNode.rowIndex].cells[1].innerText;
    window.location.href = "/Marketing/RevenueSharing/RsRentAmount/Details?Id=" + rowId;
}

function rs_rent_amount_table_header_editor_edit(e) {
    var rowId = $(e).closest('tr').children('.hidden').text();
    window.location.href = "/Marketing/RevenueSharing/RsRentAmount/Edit?Id=" + rowId;
}


function rs_rent_amount_table_header_editor_remove(e) {
    var rowId = table.rows[e.parentNode.parentNode.rowIndex].cells[1].innerText;
    let datas = {
        rentId: rowId
    }
    ...
    Swal.fire({
        title: "Confirmation",
        text: 'Are you sure want to delete this Rent Amount?',
        type: "question",
        icon: 'question',
        html: '',
        showCancelButton: !0,
        ...
        if (ok.value) {
            $.ajax({
                url: '/Marketing/RevenueSharing/RsRentAmount/onDelete',
                type: 'POST',
                data: param,
                dataType: 'json',
