$(document).ready(function () {
    ...
    async function getDataParamBuilding() {
        try {
            ...
            for (var i = 0; i < data.length; i++) {
                let selected = data[i].buildingName == 'Mal Artha Gading' ? ' selected' : '';
                if (!data[i].IsDeleted || data[i].buildingName == 'Mal Artha Gading') {
                    $('#building').append('<option value="' + data[i].buildingId + '"' + selected + '>' + data[i].buildingName + '</option>');
                }
            }
    ...
    async function getDataParamUnitStatus() {
        try {
            ...
            for (var i = 0; i < data.length; i++) {
                debugger;
                let selected = data[i].paramValue == 'Vacant' ? ' selected' : '';
                if (!data[i].IsDeleted || data[i].paramValue == 'Vacant') {
                    $('#status').append('<option value="' + data[i].paramID + '"' + selected + '>' + data[i].paramValue + '</option>');
                }
            }
...
function backToList() {
    window.location.href = '/Marketing/DataUnitPuschart';
}
