...
$(document).ready(function () {
    getAjaxDataStandardCharge("", "non_standard_charge_table_header", "NonStandard/GetDataAll", AddNew);
...
var table = document.getElementById("non_standard_charge_table_header");

function non_standard_charge_table_header_editor_view(e) {
    //var rowId = table.rows[e.parentNode.parentNode.rowIndex].cells[1].innerText;
    var rowId = table.rows[e.parentNode.parentNode.rowIndex].cells[1].innerText;
    window.location.href = "/Marketing/Rent/RentCharge/NonStandard/Details?Id=" + rowId;
}

function non_standard_charge_table_header_editor_edit(e) {
    var rowId = $(e).closest('tr').children('.hidden').text();
    window.location.href = "/Marketing/Rent/RentCharge/NonStandard/Edit?Id=" + rowId;
}


function non_standard_charge_table_header_editor_remove(e) {
    //var rowId = table.rows[e.parentNode.parentNode.rowIndex].cells[1].innerText;
    var rowId = table.rows[e.parentNode.parentNode.rowIndex].cells[1].innerText;
    //var param = { rowId }
    let datas = {
    ...
    Swal.fire({
        title: "Confirmation",
        text: 'Are you sure want to delete this Non Standard Charge?',
        type: "question",
        icon: 'question',
        html: '',
        showCancelButton: !0,
        ...
        if (ok.value) {
            $.ajax({
                url: '/Marketing/Rent/RentCharge/NonStandard/onDelete',
                type: 'POST',
                data: param,
                dataType: 'json',
