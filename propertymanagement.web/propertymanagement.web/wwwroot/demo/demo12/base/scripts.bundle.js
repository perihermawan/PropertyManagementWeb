...
//Build Approval
var buildViewer = function (elementId, options) {
    //== Main object
    var the = this;
    var init = false;
    ...
    var Plugin = {
        ...
        init: function (options) {
            ...
            the.approvalId = the.options.approvalId;
            the.approvalStatus = the.options.approvalStatus;
        },
        ...
        build: function () {
            $.ajax({
                ...
                data: JSON.stringify([the.options.approvalId]),
                success: function (r) {
                    if (r.status.success) {
                        ...
                        var approvalId = the.options.approvalId;
                        var output = '';
                                    ...
                                    if (dData.details.length > 1) {
                                        $.each(dData.details, function (i, data) {
                                            if (data.status_name == "REQUESTED") {
                                                output += ` <div class="col-lg-2 detail-view" approval-detailId="` + data.id + `" >`;
                                                output += ` ` + data.app_fullname + ` </br>`;
                                                output += ` <div class="btn-group m-btn-group m-btn-group--pill" role="group" aria-label="...">
                                                                                                         <button type="button" class="m-btn btn btn-sm btn-primary row-approve">Approve</button>
                                                                                                         <button type="button" class="m-btn btn btn-sm btn-warning row-reject">Reject</button>`;
                                                output += ` </div>`;
                                                output += ` </div>`;
                                            } else {
                                                output += ` <div class="col-lg-2 detail-view" approval-detailId="` + data.id + `" >`;
                                        ...
                                        if (dData.details[0].status_name == "REQUESTED") {
                                            output += ` <div class="col-lg-12 detail-view" approval-detailId="` + dData.details[0].id + `">`;
                        ...
                        element.html(output);
                        element.find('.row-approve').click(function () {
                            the.options.onApprove($(this).closest('div.detail-view').attr('approval-detailId'),the);
                        });
                        the.element.find('.row-reject').click(function () {
                            the.options.onReject($(this).closest('div.detail-view').attr('approval-detailId'),the);
                        });

                    }
                },
                error: function (e, t, s) {
                    mUtil.addEvent(element, 'error', Plugin.ajaxError);
                }


            });
            mUtil.addEvent(element, 'approvedOn', Plugin.approvedOn);
            mUtil.addEvent(element, 'rejectedOn', Plugin.rejectedOn);
        },

        /**
         * Handles offcanvas click ajaxErrpr
         */
        ajaxError: function () {

        },
        /**
         * Handles toggle click approvedOn
         */
        approvedOn: function () {
            return the;
        },
     ...
     * Approved on 
     */
    the.approvedOn = function () {
        return Plugin.approvedOn();
    };

    /**
     * Toggle off 
     */
    the.rejectedOn = function () {
        return Plugin.rejectedOn();
    };
...
var mLayout = function () {
    ...
    $('.page-sidebar').on('click', ' li > a.ajaxify', function (e) {
        ...
        var menuContainer = $('.m-aside-menu ul');

        menuContainer.children('li.m-menu__item--active').removeClass('active');
        menuContainer.children('li.m-menu__item--open').removeClass('m-menu__item--open');

        $(this).parents('li').each(function () {
            $(this).addClass('m-menu__item--active');
            $(this).children('.m-menu__item').addClass('m-menu__item--open');
        });
        $(this).parents('li').addClass('m-menu__item--active');
