...
namespace propertymanagement.web.Areas.Marketing.Controllers.PaymentSchedule
{
    ...
    {
        ILogger<RsController> logger;

        string url = ApiUrl.APIUrl;
        private readonly ApiClientFactory _client;

        public RsController(ApiClientFactory client, ILogger<RsController> logger)
        {
            this.logger = logger;
            this._client = client;
        }
        ...
        public async Task<IActionResult> GetDataAll(string field, string op, string value, string type)
        {
            ...
            try
            {
                ...
                sts = response.Status;
                response.IsEdit = SessionManager.PermissionSession.Where(a => a.Description == "RS Payment Schedule Edit").ToList().Count > 0 ? "Yes" : "No";
                response.IsAdd = SessionManager.PermissionSession.Where(a => a.Description == "RS Payment Schedule Add").ToList().Count > 0 ? "Yes" : "No";
                response.IsDelete = SessionManager.PermissionSession.Where(a => a.Description == "RS Payment Schedule Delete").ToList().Count > 0 ? "Yes" : "No";
            }
            catch (Exception e)
            {
                msg = e.Message;
        ...
        public async Task<IActionResult> GetMagPortionFromProgressiveRs(decimal omsetAmount, string rentId)
        {
            ...
            try
            {
                response = await _client.GetApiResponse<decimal>($"{url}/GetMagPortionFromProgressiveRs?rentId=" + rentId + "&omsetAmount=" + omsetAmount);

                sts = response.Status;
                msg = response.Message;
                data = response.Data;
                return Json(new { status = sts, message = msg, data = data });
            }
            catch (Exception ex)
            {
                return Json(new { status = "false", message = ex.Message });
