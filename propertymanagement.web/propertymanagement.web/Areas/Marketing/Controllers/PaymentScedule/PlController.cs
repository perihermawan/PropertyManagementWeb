...
namespace propertymanagement.web.Areas.Marketing.Controllers.PaymentSchedule
{
    ...
    {
        ILogger<PlController> logger;

        string url = ApiUrl.APIUrl;
        private readonly ApiClientFactory _client;

        public PlController(ApiClientFactory client, ILogger<PlController> logger)
        {
            this.logger = logger;
            this._client = client;
        }
        ...
        public async Task<IActionResult> GetDataAll(string field, string op, string value)
        {
            ...
            try
            {
                response = await _client.GetApiResponse<List<PaymentScheduleMfViewModel>>($"{url}/GetPaymentScheduleMfList?field=" + field + "&op=" + op + "&value=" + value + "&type=PL");
                msg = response.Message;
                sts = response.Status;
                response.IsEdit = SessionManager.PermissionSession.Where(a => a.Description == "PL Payment Schedule Edit").ToList().Count > 0 ? "Yes" : "No";
                response.IsAdd = SessionManager.PermissionSession.Where(a => a.Description == "PL Payment Schedule Add").ToList().Count > 0 ? "Yes" : "No";
                response.IsDelete = SessionManager.PermissionSession.Where(a => a.Description == "PL Payment Schedule Delete").ToList().Count > 0 ? "Yes" : "No";
            }
            catch (Exception e)
            {
                msg = e.Message;
        ...
        public async Task<IActionResult> OnSave(string dataParam)
        {
            ...
            try
            {
                ...
                sts = response.Status;
                msg = response.Message;
                return Json(new { status = sts, message = msg, url = "/Marketing/PaymentSchedule/PL" });
            }
            catch (Exception ex)
            {
