...
{
    [Area("Marketing")]
    public class MfController : BaseController
    {
        ILogger<MfController> logger;
        string url = ApiUrl.APIUrl;
        private readonly ApiClientFactory _client;

        public MfController(ApiClientFactory client, ILogger<MfController> logger)
        {
            this.logger = logger;
            this._client = client;
        }
        ...
        public async Task<IActionResult> GetDataAll()
        {
            ...
            try
            {
                ...
                sts = response.Status;
                response.IsEdit = SessionManager.PermissionSession.Where(a => a.Description == "Marketing MF Activation Edit").ToList().Count > 0 ? "Yes" : "No";
                response.IsAdd = SessionManager.PermissionSession.Where(a => a.Description == "Marketing MF Activation Add").ToList().Count > 0 ? "Yes" : "No";
                response.IsDelete = SessionManager.PermissionSession.Where(a => a.Description == "Marketing MF Activation Delete").ToList().Count > 0 ? "Yes" : "No";
            }
            catch (Exception e)
            {
                msg = e.Message;
        ...
        public async Task<IActionResult> GetMFOutlet(string mode)
        {
            string msg = "";
            string sts = "";
            var response = new ApiResponse();
        ...
        public async Task<IActionResult> Details(string id, string code)
        {
            ...
            try
            {
                ...
                if (code == "Rent")
                {
                    ...
                    {
                        TenantMgt.tenantRent = (List<ViewTenantMgtRentModel>)response.Data;
                    }
                }
                else if (code == "Sales")
                {
                    ...
                    if (response.Data != null)
                    {
                        TenantMgt.tenantSales = (List<ViewTenantMgtSalesModel>)response.Data;
                    }
                }
        ...
        public async Task<IActionResult> Create(List<TenantMgt> data)
        {
            ...
            try
            {
                ...
                logger.LogInformation(LogLibrary.Logging_Debug_Start("Edit", $"{url}/ExecuteSP/sp_ins_JsonPaymentScheduleMsMFCharge/{user}", "", ""));
                var paymentModel = JsonConvert.SerializeObject(paymentMf);
                response = await _client.PostApiResponse<string>($"{url}/ExecuteSP/sp_ins_JsonPaymentScheduleMsMFCharge/{user}", paymentModel);
                logger.LogInformation(LogLibrary.Logging_Debug_End("Edit", $"{url}/ExecuteSP/sp_ins_JsonPaymentScheduleMsMFCharge/{user}", "", "", response.Status, JsonConvert.SerializeObject(response.Data)));

                sts = response.Status;
                msg = response.Message;
                return Json(new { status = sts, message = msg, url = "/Marketing/Mf" });
            }
            catch (Exception ex)
            {
