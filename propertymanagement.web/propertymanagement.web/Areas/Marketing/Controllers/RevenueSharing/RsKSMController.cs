...
namespace propertymanagement.web.Areas.Marketing.Controllers.RevenueSharing
{
    ...
    public class RsKSMController : Controller
    {
        ILogger<RsKSMController> logger;
        string url = ApiUrl.APIUrl;
        private readonly ApiClientFactory _client;

        public RsKSMController(ApiClientFactory client, ILogger<RsKSMController> logger)
        {
            this.logger = logger;
            this._client = client;
        }
        ...
        {
            return View("/Areas/Marketing/Views/RevenueSharing/RsKSM/Index.cshtml");
        }
        ...
        public async Task<IActionResult> GetDataAll()
        {
            ...
            try
            {
                logger.LogInformation(LogLibrary.Logging_Debug_Start("GetRsKsmList", $"{url}/GetRsKsmList", "", ""));
                response = await _client.GetApiResponse<List<RsKsmViewModel>>($"{url}/GetRsKsmList");
                logger.LogInformation(LogLibrary.Logging_Debug_End("GetRsKsmList", $"{url}/GetRsKsmList", "", "", response.Status, JsonConvert.SerializeObject(response.Data)));
                msg = response.Message;
                sts = response.Status;
                response.IsEdit = SessionManager.PermissionSession.Where(a => a.Description == "Revenue Sharing KSM Edit").ToList().Count > 0 ? "Yes" : "No";
                response.IsAdd = SessionManager.PermissionSession.Where(a => a.Description == "Revenue Sharing KSM Add").ToList().Count > 0 ? "Yes" : "No";
                response.IsDelete = SessionManager.PermissionSession.Where(a => a.Description == "Revenue Sharing KSM Delete").ToList().Count > 0 ? "Yes" : "No";
            }
            catch (Exception e)
            {
                msg = e.Message;
        ...
        // GET: RsKSMController/Details/5
        public async Task<IActionResult> Details(string rentId)
        {
            ...
            return View("/Areas/Marketing/Views/RevenueSharing/RsKSM/Edit.cshtml");
        }

        // GET: RsKSMController/Create
        public async Task<IActionResult> Create()
        {
            ...
            return View("/Areas/Marketing/Views/RevenueSharing/RsKSM/Add.cshtml");
        }

        // GET: RsKSMController/Edit/5
        public async Task<IActionResult> Edit(string rentId)
        {
            var OutletType = await GetDataOutletType();
            var Lob = await GetDataLob();
            var ChargeTo = await GetDataChargeTo();
        ...
        // POST: RsKSMController/Create
        [HttpPost]
        public async Task<IActionResult> ValidateKSM(RsKsmModel data)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> OnCreate(RentUnitTelp data)
        {
            var user = SessionManager.UserId;
            var response = new ApiResponse();
            string msg = "";
            ...
            try
            {
                ...
                sts = response.Status;
                msg = response.Message;
                return Json(new { status = sts, message = msg, url = "/Marketing/RevenueSharing/RsKSM" });
            }
            catch (Exception ex)
            {
                return Json(new { status = "false", message = ex.Message });
        ...
        [HttpGet]
        public async Task<IActionResult> GetDataUnitAll(string field1, string op1, string value1, string field2, string op2, string value2, string field3, string op3, string value3)
        {
            string msg = "";
            string sts = "";
            var response = new ApiResponse();
            try
            {
                logger.LogInformation(LogLibrary.Logging_Debug_Start("GetDataUnitRsKsmList", $"{url}/GetDataUnitRsKsmList?field1={field1}&op1={op1}&value1={value1}&field2={field2}&op2={op2}&value2={value2}&field3={field3}&op3={op3}&value3={value3}", "", ""));
                response = await _client.GetApiResponse<List<RentUnitListModel>>($"{url}/GetDataUnitRsKsmList?field1={field1}&op1={op1}&value1={value1}&field2={field2}&op2={op2}&value2={value2}&field3={field3}&op3={op3}&value3={value3}");
                logger.LogInformation(LogLibrary.Logging_Debug_End("GetDataUnitRsKsmList", $"{url}/GetDataUnitRsKsmList?field1= {field1} &op1= {op1} &value1= {value1} &field2= {field2} &op2= {op2} &value2= {value2} &field3= {field3} &op3= {op3} &value3= {value3}", "", "", response.Status, JsonConvert.SerializeObject(response.Data)));
            }
            catch (Exception e)
            {
                msg = e.Message;
